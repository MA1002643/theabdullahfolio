...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/last_synced-2026--07--28-ff6d05?style=flat-square" alt="Last synced" />
</p>
<!-- QUALITY-BADGES:END -->
...
| Homepage causeway scene | The hero backdrop's own lanterns, their doubles in the still water and the cold vanishing-point mist repainted as additive glows riding two-octave seeded value noise, plus fireflies drifting over the water at the frame edges. **Procedural only — no ambient video ships for this route**: on the site's LCP-critical entry page a zero-byte layer that loops by construction and stays crisp at any viewport beats a decode that can fail. The 15 lantern positions are **measured** off the shipped plate by `scripts/scene/detect-lights.mjs` (blobs thresholded on warmth `R−B`, not luminance, so the bright-but-cold mist is not mistaken for a flame; then filtered on a near-clipped flame core, which rejects wet-stone specular and water smear) rather than eyeballed, because the ignite runs a wavefront along that same perspective line. Tiered **full / lite / still** — the lite rung (Save-Data, `deviceMemory ≤ 4`, `hardwareConcurrency ≤ 4`) drops the mist, the reflections and 9 of 14 fireflies and falls to 20fps. Reflections are drawn vertically stretched, since still water smears a point light | `home/HomeSceneGlow.jsx` · `home/homeSceneLights.js` · `scripts/scene/gen-home-hero.mjs` · `scripts/scene/detect-lights.mjs` |
| Causeway path ignition on arrival | A wavefront sweeps the wet stone from below the bottom edge to the vanishing point, its width tracking the causeway's own perspective, each lantern catching as it passes, then blooming into the mist and handing back to the baked plate before the canvas unmounts (~2.5 s). Lanterns catch by **depth derived from their own position**, not by hand-numbered index, so both rows light in true near-to-far order and moving a lantern re-times the sequence. The homepage is the one route that never scrolls, so it has no scroll indicator to say "there is more this way" — the background says it instead, by sending the light inward. Frame deltas are clamped so a backgrounded tab pauses the sequence rather than skipping it. **The travelling light is the headline's colour**: the wavefront and the vanishing-point bloom hold one hue, `#ff6d05` — the stroke of the MUHAMMAD ABDULLAH `h1`, and the same identity call the ripple rings make — with alpha alone carrying the falloff, in place of the near-white → yellow → amber temperature ramp that made an additive light read yellow whatever its mid stop said. The lanterns catching keep their filmed warmth on purpose: that flare decays back into the ambient halo and the flame loop, so an orange one would pop and settle wrong | `home/HomePathIgnite.jsx` |
